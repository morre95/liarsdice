...
import { renderModel, resolveMatchId, spectatorSocketUrl } from "../public/spectator.js";
...
test("spectator assets exist and model collects bids, talk, reveal, and reasoning", () => {
  ...
  assert.match(html, /spectator\.js/);
  assert.match(html, /id="lobby-view"/);
  assert.match(html, /id="match-form"/);
...
test("lobby routing uses explicit match query parameters and secure WebSockets", () => {
  assert.equal(resolveMatchId({ search: "" }), null);
  assert.equal(resolveMatchId({ search: "?match_id=final-table" }), "final-table");
  assert.equal(spectatorSocketUrl({ protocol: "https:", host: "dice.example" }, "final table"),
    "wss://dice.example/spectate/final%20table");
  assert.equal(spectatorSocketUrl({ protocol: "http:", host: "localhost:8080" }, "one"),
    "ws://localhost:8080/spectate/one");
  assert.equal(renderModel({ ...state, phase: "finished" }, { type: "snapshot", status: "finished" }).finished, true);
});
